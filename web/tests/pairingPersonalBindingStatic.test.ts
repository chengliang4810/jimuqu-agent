...
assert.ok(api.includes("'connected' | 'degraded'"), 'pairing channel state should expose outbound degradation')
...
assert.ok(control.includes("channelState.value === 'degraded'"), 'outbound degradation should use a dedicated warning state')
assert.ok(control.includes("return 'warning'"), 'outbound degradation should render with warning color')

...
const TAG_FULL = 'SolonClaw Agent'
const TAG_MID = 'SolonClaw Agent'
const TAG_TINY = 'SolonClaw'
const HIDE_BELOW = 34
...
const centerIn = (s: string, w: number) => {
  const f = clip(s, w)
  const extra = Math.max(0, w - f.length)
  const left = extra >> 1
...
const ruleIn = (label: string, w: number) => {
  ...
  const extra = Math.max(0, w - f.length - 2)
  const left = extra >> 1
      ...
      {wide && (
        <Box flexDirection="column" marginRight={2} width={leftW}>
          ...
          <Text color={t.color.accent}>
            ...
            <Text color={t.color.muted}> · SolonClaw</Text>
          </Text>
      ...
      <Box flexDirection="column" width={w}>
        ...
        {typeof info.update_behind === 'number' && info.update_behind > 0 && (
          <Text bold color={t.color.warn}>
            ...
            <Text bold color={t.color.warn}>
              {info.update_command || 'solonclaw update'}
            </Text>
            <Text bold={false} color={t.color.warn} dimColor>
              {' '}
              to update

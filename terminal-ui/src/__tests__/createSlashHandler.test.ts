...
describe('createSlashHandler', () => {
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('solonclaw TUI Status', 'Status')
    })
  })
        ...
        providers: [
          {
            configured: true,
            current: true,
            models: ['mimo-v2.5-pro', 'mimo-v2.5-fast'],
  ...
  it('opens the setup panel in-place without launching an external command', () => {
    ...
    expect(getOverlayState().channelSetup).toBe(false)
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
  ...
  it('opens model setup in-place without launching an external command', () => {
    ...
    expect(getOverlayState().setupPanel).toBe(false)
    expect(getOverlayState().modelPicker).toBe(true)
    expect(getOverlayState().channelSetup).toBe(false)
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    expect(ctx.gateway.rpc).not.toHaveBeenCalled()
  ...
  it('opens domestic channel setup in-place without launching an external command', () => {
    const ctx = buildCtx()

    expect(createSlashHandler(ctx)('/setup gateway')).toBe(true)
    expect(getOverlayState().setupPanel).toBe(false)
    expect(getOverlayState().channelSetup).toBe(true)
    expect(getOverlayState().modelPicker).toBe(false)
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    expect(ctx.gateway.rpc).not.toHaveBeenCalled()
  })
  ...
  it('honors TUI picker session scope without adding --global', async () => {
    ...
    const ctx = buildCtx({
      gateway: {
        ...buildGateway(),
        rpc: vi.fn(() => Promise.resolve({ value: 'mimo-v2.5-pro' }))
      }
    })

    expect(createSlashHandler(ctx)(`/model mimo-v2.5-pro --provider default ${TUI_SESSION_MODEL_FLAG}`)).toBe(true)
    expect(ctx.gateway.rpc).toHaveBeenCalledWith('config.set', {
      key: 'model',
      session_id: 'sid-abc',
      value: 'mimo-v2.5-pro --provider default'
    })
  })
  ...
  it('passes /reset through to the session lifecycle instead of backend slash exec', () => {
    const ctx = buildCtx()
  ...
  it('/voice on reports unavailable backend details instead of a generic disabled message', async () => {
    const rpc = vi.fn(() =>
      Promise.resolve({
        available: false,
  ...
  it('falls through to command.dispatch for skill commands and sends the message', async () => {
      ...
      gateway: {
        gw: {
          ...
          request: vi.fn((method: string) => {
            ...
            if (method === 'command.dispatch') {
              return Promise.resolve({ type: 'skill', message: skillMessage, name: 'solonclaw-agent-dev' })
            }
    ...
    const h = createSlashHandler(ctx)
    expect(h('/solonclaw-agent-dev')).toBe(true)
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('⚡ loading skill: solonclaw-agent-dev')
    })
  ...
  it('handles command dispatch payloads returned directly by slash.exec', async () => {
    ...
    const request = vi.fn((method: string) => {
      if (method === 'slash.exec') {
        return Promise.resolve({
          message: 'complete all steps and report back',
          notice: 'goal set',
          type: 'send'
        })
      }
    ...
    expect(createSlashHandler(ctx)('/goal complete all steps and report back')).toBe(true)

    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('goal set')
    })
    expect(ctx.transcript.send).toHaveBeenCalledWith('complete all steps and report back')
  ...
  it('/history pages the current TUI transcript (user + assistant)', () => {
    ...
    expect(body).toContain('[solonclaw #2]')
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    const rpc = vi.fn(() => Promise.resolve({ file: '/tmp/solonclaw_conversation_test.json' }))
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('conversation saved to: /tmp/solonclaw_conversation_test.json')
    })
  })
  ...
  it('reports /undo RPC failures instead of failing silently', async () => {
    patchUiState({ sid: 'sid-abc' })
    const rpc = vi.fn(() => Promise.reject(new Error('undo unavailable')))
    const ctx = buildCtx({ gateway: { ...buildGateway(), rpc } })

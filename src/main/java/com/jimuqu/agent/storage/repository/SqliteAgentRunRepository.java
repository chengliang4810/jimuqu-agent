...
public class SqliteAgentRunRepository implements AgentRunRepository {
    ...
    public AgentRunRecord findRun(String runId) throws Exception {
        Connection connection = database.openConnection();
        try {
            PreparedStatement statement = connection.prepareStatement("select * from agent_runs where run_id = ?");
    ...
    public List<AgentRunRecord> listBySession(String sessionId, int limit) throws Exception {
        ...
        Connection connection = database.openConnection();
        try {
            PreparedStatement statement = connection.prepareStatement("select * from agent_runs where session_id = ? order by started_at desc limit ?");
    ...
    public List<AgentRunEventRecord> listEvents(String runId) throws Exception {
        ...
        Connection connection = database.openConnection();
        try {
            PreparedStatement statement = connection.prepareStatement("select * from agent_run_events where run_id = ? order by created_at asc");
            statement.setString(1, runId);
            ResultSet resultSet = statement.executeQuery();
            try {
                while (resultSet.next()) {
    ...
    public void pruneBefore(long beforeEpochMillis) throws Exception {
        Connection connection = database.openConnection();
        try {
            PreparedStatement deleteEvents = connection.prepareStatement("delete from agent_run_events where run_id in (select run_id from agent_runs where started_at < ?)");
            deleteEvents.setLong(1, beforeEpochMillis);
            deleteEvents.executeUpdate();
            deleteEvents.close();

            PreparedStatement deleteRuns = connection.prepareStatement("delete from agent_runs where started_at < ?");

...
public class SelfUpdateLauncher {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < 30; i++) {
            try {
                Files.move(downloadedJar.toPath(), targetJar.toPath(), StandardCopyOption.REPLACE_EXISTING, StandardCopyOption.ATOMIC_MOVE);
                replaced = true;
                break;
            } catch (Exception ignored) {
                try {
                    Files.copy(downloadedJar.toPath(), targetJar.toPath(), StandardCopyOption.REPLACE_EXISTING);

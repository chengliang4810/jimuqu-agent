...
public final class SecretRedactor {
    ...
    private static final Pattern KEY_VALUE = Pattern.compile("(?i)(api[_-]?key|token|secret|password|authorization|client[_-]?secret)(\\s*[:=]\\s*)([^\\s,;\"'}]+)");
    ...
    public static String redact(String text, int maxLength) {
        ...
        if (result.length() > limit) {
            return result.substring(0, limit) + "\n...[truncated, totalLength=" + result.length() + "]";
        }
        return result;
    }

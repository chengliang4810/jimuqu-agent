...
public class DisplaySettingsService {
    ...
    public DisplaySettingsService(AppConfig appConfig,
    ...
    public String resolveToolProgress(PlatformType platform) {
        String configured = channelConfig(platform) == null ? null : channelConfig(platform).getToolProgress();
    ...
    public boolean isReasoningVisible(String sourceKey, PlatformType platform) {
        if (globalSettingRepository != null && StrUtil.isNotBlank(sourceKey)) {
            try {
                String stored = globalSettingRepository.get(AgentSettingConstants.DISPLAY_REASONING_VISIBLE_PREFIX + sourceKey);

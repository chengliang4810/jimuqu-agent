...
public class DefaultContextBudgetService implements ContextBudgetService {
    ...
    public ContextBudgetDecision decide(SessionRecord session, String systemPrompt, String userMessage, AppConfig.LlmConfig resolved) {
        int contextWindow = resolved == null || resolved.getContextWindowTokens() <= 0
        ...
        int estimated = estimate(systemPrompt) + estimate(userMessage) + estimate(session == null ? null : session.getNdjson());
        ...
        decision.setShouldCompress(appConfig.getCompression().isEnabled() && estimated >= threshold);
        decision.setReason(decision.isShouldCompress()
                ? "预计上下文已达到压缩阈值"
                : "预计上下文未达到压缩阈值");
        return decision;
    }

package com.jimuqu.agent.skillhub.service;

import cn.hutool.core.date.DateUtil;
import cn.hutool.core.io.FileUtil;
import cn.hutool.core.util.StrUtil;
import com.jimuqu.agent.core.service.SkillGuardService;
import com.jimuqu.agent.skillhub.model.Finding;
...
public class DefaultSkillGuardService implements SkillGuardService {
    ...
    private static final Set<String> TRUSTED_REPOS = new LinkedHashSet<String>(
            java.util.Arrays.asList("openai/skills", "anthropics/skills")
    );

    private static final List<ThreatPattern> THREAT_PATTERNS = java.util.Arrays.asList(
            new ThreatPattern("hardcoded_secret", "critical", "credential_exposure", "(api[_-]?key|token|secret|password)\\s*[=:]\\s*[\"'][A-Za-z0-9+/=_-]{20,}", "possible hardcoded secret"),
            new ThreatPattern("embedded_private_key", "critical", "credential_exposure", "-----BEGIN\\s+(RSA\\s+)?PRIVATE\\s+KEY-----", "embedded private key"),
            new ThreatPattern("curl_pipe_shell", "critical", "supply_chain", "curl\\s+[^\\n]*\\|\\s*(ba)?sh", "curl piped to shell"),
            new ThreatPattern("wget_pipe_shell", "critical", "supply_chain", "wget\\s+[^\\n]*-O\\s*-\\s*\\|\\s*(ba)?sh", "wget piped to shell"),
            new ThreatPattern("dangerous_rm", "critical", "destructive", "rm\\s+-rf\\s+/", "recursive delete from root"),
            new ThreatPattern("sudo_usage", "high", "privilege_escalation", "\\bsudo\\b", "uses sudo"),
            new ThreatPattern("python_subprocess", "medium", "execution", "subprocess\\.(run|call|Popen|check_output)\\s*\\(", "Python subprocess execution"),
            new ThreatPattern("python_os_system", "high", "execution", "os\\.system\\s*\\(", "os.system shell execution"),
            new ThreatPattern("prompt_injection_ignore", "critical", "injection", "ignore\\s+(?:\\w+\\s+)*(previous|all|above|prior)\\s+instructions", "prompt injection ignore instructions"),
            new ThreatPattern("role_hijack", "high", "injection", "you\\s+are\\s+(?:\\w+\\s+)*now\\s+", "role hijack"),
            new ThreatPattern("shell_rc_mod", "medium", "persistence", "\\.(bashrc|zshrc|profile|bash_profile|bash_login|zprofile|zlogin)\\b", "references shell startup file"),
            new ThreatPattern("agent_config_mod", "critical", "persistence", "AGENTS\\.md|CLAUDE\\.md|\\.cursorrules|\\.clinerules", "references agent config files")
    );
    ...
    public String formatReport(ScanResult result) {
        ...
        buffer.append("Scan: ").append(result.getSkillName())
                .append(" (").append(result.getSource()).append("/").append(result.getTrustLevel()).append(")")
                .append(" Verdict: ").append(result.getVerdict().toUpperCase(Locale.ROOT));
        for (Finding finding : result.getFindings()) {
            buffer.append('\n')
                    .append("  ")
                    .append(StrUtil.padAfter(finding.getSeverity().toUpperCase(Locale.ROOT), 8, ' '))
                    .append(" ")
                    .append(StrUtil.padAfter(finding.getCategory(), 14, ' '))
                    .append(" ")
                    .append(finding.getFile()).append(":").append(finding.getLine())
                    .append(" \"").append(finding.getMatch()).append("\"");
        }
        InstallDecision decision = shouldAllowInstall(result, false);
        buffer.append('\n').append("Decision: ")
                .append(decision.isAllowed() ? "ALLOWED" : decision.isRequiresConfirmation() ? "NEEDS CONFIRMATION" : "BLOCKED")
                .append(" - ").append(decision.getReason());
    ...
    private List<Finding> checkStructure(File skillDir) {
        ...
        for (File file : files) {
            ...
            if (file.length() > MAX_SINGLE_FILE_KB * 1024L) {
                findings.add(finding("oversized_file", "medium", "structural", rel, 0, (file.length() / 1024L) + "KB", "single file too large"));
            }
            String ext = FileUtil.extName(file).toLowerCase(Locale.ROOT);
            if (java.util.Arrays.asList("exe", "dll", "so", "dylib", "bin", "msi", "dmg").contains(ext)) {
                findings.add(finding("binary_file", "critical", "structural", rel, 0, ext, "binary file in skill"));
            }
        }

        if (fileCount > MAX_FILE_COUNT) {
            findings.add(finding("too_many_files", "medium", "structural", "(directory)", 0, String.valueOf(fileCount), "too many files"));
        }
        if (totalSize > MAX_TOTAL_SIZE_KB * 1024L) {
            findings.add(finding("oversized_skill", "high", "structural", "(directory)", 0, (totalSize / 1024L) + "KB", "skill too large"));
        }
        return findings;
    }
    ...
    private String buildSummary(ScanResult result) {
        ...
        return result.getSkillName() + ": " + result.getVerdict()
                + " - " + result.getFindings().size()
                + " finding(s) in " + String.join(", ", categories);
    }

    private Finding finding(String patternId,
                            String severity,
                            String category,
                            String file,
                            int line,
    ...
    private String relativePath(File root, File file) {
        ...
        return absolute.startsWith(base) ? absolute.substring(base.length()).replace(File.separatorChar, '/') : file.getName();
    }
    ...
    private static class ThreatPattern {
        ...
        private ThreatPattern(String patternId, String severity, String category, String regex, String description) {

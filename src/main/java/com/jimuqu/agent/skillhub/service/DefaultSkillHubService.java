...
import com.jimuqu.agent.skillhub.source.OfficialSkillSource;
import com.jimuqu.agent.skillhub.source.SkillSource;
...
public class DefaultSkillHubService implements SkillHubService {
    ...
    public DefaultSkillHubService(File repoRoot,
                                  File skillsDir,
    ...
    public SkillBrowseResult browse(String sourceFilter, int page, int pageSize) throws Exception {
        List<SkillMeta> all = collectFromSources("", sourceFilter, Math.max(pageSize * Math.max(page, 1), pageSize));
    ...
    @Override
    public HubInstallRecord install(String identifier, String category, boolean force) throws Exception {
        for (SkillSource source : sources()) {
            if (!matchesSourceFilter(source, identifier)) {
                continue;
            }
    ...
    public List<HubInstallRecord> check(String name) throws Exception {
        ...
        for (HubInstallRecord record : stateStore.listInstalled()) {
            ...
            copy.getMetadata().put("status", record.getContentHash().equals(latestHash) ? "up_to_date" : "update_available");
    ...
    public List<HubInstallRecord> update(String name, boolean force) throws Exception {
        ...
        for (HubInstallRecord record : check(name)) {
            ...
            HubInstallRecord installed = install(record.getIdentifier(), deriveCategory(record.getInstallPath()), force);
    ...
    public List<ScanResult> audit(String name) throws Exception {
        ...
        for (HubInstallRecord record : stateStore.listInstalled()) {
            ...
            File installDir = FileUtil.file(skillsDir, record.getInstallPath().replace('/', File.separatorChar));
    ...
    public String uninstall(String name) {
        ...
        stateStore.appendAuditLog("UNINSTALL", name, record.getSource(), record.getTrustLevel(), "n/a", "user_request");
    ...
    private List<SkillMeta> collectFromSources(String query, String sourceFilter, int limit) throws Exception {
        List<SkillMeta> results = new ArrayList<SkillMeta>();
        for (SkillSource source : sources()) {
            if (!"all".equals(sourceFilter) && !source.sourceId().equals(sourceFilter) && !"official".equals(source.sourceId())) {
                continue;
            }
        ...
        for (SkillMeta meta : results) {
            ...
            if (current == null || trustRank(meta.getTrustLevel()) > trustRank(current.getTrustLevel())) {
                unique.put(meta.getName(), meta);
            }
        }
        List<SkillMeta> deduped = new ArrayList<SkillMeta>(unique.values());
        Collections.sort(deduped, new Comparator<SkillMeta>() {
            @Override
            public int compare(SkillMeta left, SkillMeta right) {
                int rank = trustRank(right.getTrustLevel()) - trustRank(left.getTrustLevel());
                if (rank != 0) {
                    return rank;
                }
    ...
    private List<SkillSource> sources() {
        ...
        sources.add(new ClaudeMarketplaceSkillSource(httpClient, gitHubAuth, gitHubSkillSource, stateStore));

...
public class LobeHubSkillSource implements SkillSource {
    ...
    public List<SkillMeta> search(String query, int limit) throws Exception {
        ...
        for (int i = 0; i < agents.size(); i++) {
            ...
            String identifier = StrUtil.blankToDefault(agent.get("identifier").getString(), metaNode.get("title").getString().toLowerCase().replace(" ", "-"));
    ...
    public SkillBundle fetch(String identifier) throws Exception {
        ...
        ONode agent = ONode.ofJson(httpClient.getText("https://chat-agents.lobehub.com/" + agentId + ".json", null));
        if (!agent.isObject()) {
            return null;
        }
    ...
    public static String convertToSkillMd(ONode agentData) {
        ...
        String identifier = StrUtil.blankToDefault(agentData.get("identifier").getString(), "lobehub-agent");
        ...
        buffer.append("description: ").append(StrUtil.blankToDefault(description, title)).append("\n");
        ...
        buffer.append(StrUtil.blankToDefault(systemRole, "Follow the original LobeHub agent instructions."));
        buffer.append("\n");

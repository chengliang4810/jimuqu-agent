...
public class HermesIndexSource implements SkillSource {
    private static final String INDEX_URL = "https://hermes-agent.nousresearch.com/docs/api/skills-index.json";
    ...
    public HermesIndexSource(SkillHubHttpClient httpClient,
    ...
    public List<SkillMeta> search(String query, int limit) throws Exception {
        ...
        for (int i = 0; i < skills.size(); i++) {
            ...
            String searchable = (item.get("name").getString() + " " + item.get("description").getString() + " " + item.get("tags").toJson()).toLowerCase();
    ...
    public SkillBundle fetch(String identifier) throws Exception {
        ...
        if (bundle != null) {
            ...
            bundle.setTrustLevel(StrUtil.blankToDefault(entry.get("trust_level").getString(), "community"));
        }
        return bundle;
    }
    ...
    public String trustLevelFor(String identifier) {
        try {
            ONode entry = findEntry(identifier);
            return entry == null ? "community" : StrUtil.blankToDefault(entry.get("trust_level").getString(), "community");
        } catch (Exception e) {
            return "community";
        }
    }
    ...
    private ONode findEntry(String identifier) throws Exception {
        ...
        for (int i = 0; i < skills.size(); i++) {
            ...
            if (identifier.equals(item.get("identifier").getString()) || normalized.equals(candidate)) {
                return item;
            }
        }
        return null;
    ...
    private SkillMeta toMeta(ONode node) {
        ...
        meta.setTrustLevel(StrUtil.blankToDefault(node.get("trust_level").getString(), "community"));
    ...
    private String stripPrefix(String identifier) {
        ...
        String[] prefixes = new String[]{"skills-sh/", "skills.sh/", "official/", "github/", "clawhub/"};

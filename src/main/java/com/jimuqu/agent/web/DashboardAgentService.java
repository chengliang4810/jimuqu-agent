...
public class DashboardAgentService {
    ...
    public DashboardAgentService(AgentProfileService agentProfileService,
    ...
    public Map<String, Object> list(String sessionId) throws Exception {
        ...
        SessionRecord session = StrUtil.isBlank(sessionId) ? null : sessionRepository.findById(sessionId);
        String active = AgentRuntimeScope.normalizeName(session == null ? null : session.getActiveAgentName());
    ...
    public Map<String, Object> activate(String name, Map<String, Object> body) throws Exception {
        ...
        sessionRepository.setActiveAgentName(sessionId, AgentRuntimeScope.DEFAULT_AGENT.equals(normalized) ? null : normalized);
    ...
    private Map<String, Object> baseProfile(AgentProfile profile, String active) {
        ...
        item.put("display_name", StrUtil.blankToDefault(profile.getDisplayName(), profile.getAgentName()));
    ...
    private List<Map<String, Object>> recentRuns(String agentName) throws Exception {
        ...
        List<SessionRecord> sessions = sessionRepository.listRecent(Math.min(Math.max(total, 100), 1000), 0);
        for (SessionRecord session : sessions) {
            for (AgentRunRecord run : agentRunRepository.listBySession(session.getSessionId(), 10)) {
    ...
    private void applyMutableFields(AgentProfile profile, Map<String, Object> body) {
        ...
        if (body.containsKey("default_model")) profile.setDefaultModel(string(body, "default_model"));
        if (body.containsKey("allowed_tools_json")) profile.setAllowedToolsJson(jsonString(body.get("allowed_tools_json")));
        if (body.containsKey("skills_json")) profile.setSkillsJson(jsonString(body.get("skills_json")));
        if (body.containsKey("memory")) profile.setMemory(string(body, "memory"));
        if (body.containsKey("enabled")) profile.setEnabled(Boolean.TRUE.equals(body.get("enabled")) || "true".equalsIgnoreCase(String.valueOf(body.get("enabled"))));
    }

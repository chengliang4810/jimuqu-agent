...
public class DashboardChatService {
    ...
    private final ConcurrentMap<String, ChatRunState> runs = new ConcurrentHashMap<String, ChatRunState>();

    public DashboardChatService(SessionRepository sessionRepository,
    ...
    public Map<String, Object> uploads(UploadedFile[] files) throws Exception {
        ...
        for (UploadedFile file : files) {
            ...
            try {
                MessageAttachment attachment = attachmentCacheService.cacheBytes(
                        PlatformType.MEMORY,
                        null,
                        file.getName(),
                        file.getContentType(),
    ...
    public Map<String, Object> startRun(ONode body) throws Exception {
        ...
        if (StrUtil.isBlank(request.input) && (request.attachments == null || request.attachments.isEmpty())) {
    ...
    public Map<String, Object> cancelRun(String runId) {
        ...
        enqueue(state, "run.failed", new LinkedHashMap<String, Object>() {
            {
        ...
        try {
            ...
            if (!state.completed) {
                if (reply != null && reply.isError()) {
                    eventSink.onRunFailed(reply.getSessionId(), new IllegalStateException(reply.getContent()));
                } else {
                    eventSink.onRunCompleted(reply == null ? state.sessionId : reply.getSessionId(),
    ...
    private GatewayMessage buildMessage(ChatRunRequest request, String sessionId) {
        GatewayMessage message = new GatewayMessage(PlatformType.MEMORY, "dashboard", sessionId, request.input);
    ...
    private String extractTitle(ChatRunRequest request) {
        ...
        if (request.conversationHistory != null) {
            for (HistoryItem item : request.conversationHistory) {
                if (item != null && "user".equalsIgnoreCase(item.role) && StrUtil.isNotBlank(item.content)) {
                    base = item.content;
                    break;
                }
            }
    ...
    private void writeEvent(OutputStream outputStream, ChatRunEvent event) throws IOException {
        ...
        outputStream.write(("data: " + ONode.serialize(event.data) + "\n\n").getBytes(StandardCharsets.UTF_8));
        outputStream.flush();
    }
    ...
    private final class DashboardRunEventSink implements ConversationEventSink {
        ...
        public void onRunFailed(String sessionId, Throwable error) {
            ...
            Map<String, Object> payload = new LinkedHashMap<String, Object>();
            payload.put("error", error == null ? "Run failed" : StrUtil.blankToDefault(error.getMessage(), error.getClass().getSimpleName()));
            enqueue(state, "run.failed", payload);
        }

        private String truncateInline(String text, int limit) {
            String normalized = StrUtil.nullToEmpty(text).replace('\r', ' ').replace('\n', ' ').trim();

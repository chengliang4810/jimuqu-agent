...
public class DashboardStaticController {
    ...
    private Object renderResource(Context context, String resourcePath) {
        ...
        if (devFile != null) {
            return new DownloadedFile(contentType(resourcePath), FileUtil.readBytes(devFile), devFile.getName()).asAttachment(false);
        }

        byte[] bytes = loadClasspathBytes(resourcePath);
        if (bytes == null) {
            context.status(404);
            return new DownloadedFile("text/plain;charset=UTF-8", new byte[0], "404.txt").asAttachment(false);
        }

        return new DownloadedFile(contentType(resourcePath), bytes, fileName(resourcePath)).asAttachment(false);
    }

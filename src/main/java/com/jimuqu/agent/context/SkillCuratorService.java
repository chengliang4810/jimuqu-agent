...
public class SkillCuratorService {
    ...
    public synchronized Map<String, Object> runOnce(boolean force) throws Exception {
        ...
        long intervalMillis = Math.max(1, appConfig.getCurator().getIntervalHours()) * 60L * 60L * 1000L;
    ...
    private Map<String, Object> reviewSkill(SkillDescriptor descriptor,
                                            Map<String, Object> skillsState,
                                            long now) {
        String name = descriptor.canonicalName();
        Map<String, Object> record = skillsState.get(name) instanceof Map
        ...
        String previousStatus = StrUtil.nullToDefault(String.valueOf(record.get("status")), "active");
    ...
    private Map<String, Object> report(Map<String, Object> state,
                                       long now,
                                       String status,
    ...
    private void writeReport(Map<String, Object> report, long now) {
        ...
        for (Object itemObj : (List<?>) report.get("items")) {
            ...
            markdown.append("- ").append(item.get("name"))
                    .append(" -> ").append(item.get("status"))
                    .append(" (").append(item.get("action")).append(")\n");
        }

...
import com.jimuqu.solon.claw.gateway.service.ProfileRuntimeBundleFactory;
...
/** 在应用装配层创建 Profile 子容器，避免网关运行时反向依赖 bootstrap 配置类。 */
public class BootstrapProfileRuntimeBundleFactory implements ProfileRuntimeBundleFactory {
    /** 主应用 Bean 容器，仅复制 Solon 已注册的注解构建器和注入器。 */
    private final AppContext rootContext;

    /** 创建显式 Profile 运行时工厂。 */
    public BootstrapProfileRuntimeBundleFactory(AppContext rootContext) {
        if (rootContext == null) {
    ...
    public ProfileRuntimeBundle create(
        ...
        try (ProfileRuntimeScope.Scope ignored =
                ProfileRuntimeScope.open(profile, home, environment, child)) {

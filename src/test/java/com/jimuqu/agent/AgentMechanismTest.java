...
public class AgentMechanismTest {
    ...
    void shouldSwitchCurrentSessionAgentAndKeepDefaultAtRuntimeRoot() throws Exception {
        ...
        assertThat(new File(env.appConfig.getRuntime().getHome(), "agents/coder/workspace")).isDirectory();
    ...
    void shouldRejectDefaultMutation() throws Exception {
        ...
        GatewayReply model = env.send("default-room", "default-user", "/agent model default gpt-5.2");
    ...
    void shouldUseAgentModelUnlessSessionOverrideExistsAndRecordSnapshot() throws Exception {
        ...
        SessionRecord session = env.sessionRepository.getBoundSession("MEMORY:model-room:model-user");
        List<AgentRunRecord> runs = env.agentRunRepository.listBySession(session.getSessionId(), 10);
        ...
        assertThat(runs.get(0).getAgentSnapshotJson()).contains("\"default_model\":\"agent-model\"");
    ...
    void shouldUseNamedAgentRoleAndWorkspace() throws Exception {
        ...
        String expectedWorkspace = FileUtil.file(env.appConfig.getRuntime().getHome(), "agents", "coder", "workspace").getAbsolutePath();
    ...
    void shouldApplyAgentToolAllowlist() throws Exception {
        ...
        AgentRuntimeScope scope = env.agentRuntimeService.resolve(env.sessionRepository.getBoundSession("MEMORY:tools-room:tools-user"));
        List<String> names = env.toolRegistry.resolveEnabledToolNames("MEMORY:tools-room:tools-user", scope);
        String joinedTools = env.toolRegistry.resolveEnabledTools("MEMORY:tools-room:tools-user", scope).toString();
    ...
    void shouldCleanSessionsWhenDashboardDeletesActiveAgent() throws Exception {
        ...
        SessionRecord session = env.sessionRepository.getBoundSession("MEMORY:delete-room:delete-user");
        ...
        assertThat(env.agentRuntimeService.resolve(updated).getEffectiveName()).isEqualTo("default");
    }
    ...
    void shouldRejectDisabledDashboardActivation() throws Exception {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(new org.assertj.core.api.ThrowableAssert.ThrowingCallable() {
            @Override
            public void call() throws Throwable {
                dashboardAgentService(env).activate("sleepy", activate);
            }
        }).isInstanceOf(IllegalArgumentException.class).hasMessageContaining("Agent 已停用");
    }
    ...
    private static DashboardAgentService dashboardAgentService(TestEnvironment env) {
        return new DashboardAgentService(env.agentProfileService, env.agentRuntimeService, env.sessionRepository, env.agentRunRepository);
    }
    ...
    private static class RecordingGateway extends FakeLlmGateway {
        ...
        public LlmResult executeOnce(SessionRecord session,
                                     ...
                                     com.jimuqu.agent.core.model.AgentRunContext runContext) throws Exception {

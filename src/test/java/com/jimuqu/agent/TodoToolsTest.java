...
import com.jimuqu.agent.tool.runtime.TodoTools;
...
public class TodoToolsTest {
    ...
    void shouldReadAndReplaceCurrentList() throws Exception {
        ...
        ONode written = ONode.ofJson(tools.todo(Arrays.asList(
    ...
    void shouldMergeByIdAndAppendNewItems() throws Exception {
        ...
        tools.todo(Arrays.asList(
                item("1", "Plan implementation", "in_progress"),
                item("2", "Verify behavior", "pending")
        ), false);
        ONode merged = ONode.ofJson(tools.todo(Arrays.asList(
        ...
        assertThat(merged.get("todos").get(0).get("content").getString()).isEqualTo("Plan implementation");
    ...
    void shouldNormalizeLikeHermes() throws Exception {
        ...
        assertThat(written.get("todos").get(1).get("content").getString()).isEqualTo("(no description)");
    }
